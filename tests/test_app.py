...
async def test_compose_mounts_expected_widgets(app):
    ...
    assert the_app.query_one(TextArea)
    assert the_app.query_one(ImageViewer)
...
async def test_on_mount_preset_image(config_path, tmp_path):
    ...
    the_app.file = img
    async with the_app.run_test():
        assert the_app.query_one(ImageViewer).display is True
        assert the_app.query_one(TextArea).display is False
...
async def test_edit_file_directory_sets_empty_text(app, sample_dir):
    ...
    await pilot.pause()
    assert the_app.query_one(TextArea).text == ""
...
async def test_edit_file_opens_image(app, tmp_path):
    ...
    await pilot.pause()
    viewer = the_app.query_one(ImageViewer)
    editor = the_app.query_one(TextArea)
...
async def test_edit_file_after_image_restores_editor(app, sample_dir, tmp_path):
    ...
    the_app.edit_file(sample_dir / "hello.py")
    await pilot.pause()
    assert the_app.query_one(TextArea).display is True
    assert the_app.query_one(ImageViewer).display is False
    assert the_app.file == sample_dir / "hello.py"
    assert the_app.query_one(TextArea).text == "print('hi')\n"
...
async def test_edit_file_language_does_not_exist_fallback(app, sample_dir, monkeypatch):
    ...
    the_app.edit_file(sample_dir / "hello.py")
    await pilot.pause()
    assert the_app.query_one(TextArea).language == ""
